...
Use direct context loading (not RAG) for authoring assistance. Style guides are small enough to fit entirely in modern LLM context windows (100k+ tokens), ensuring the AI sees all guidelines consistently. Include 5-10 examples of well-written docs for different content types. RAG risks missing critical style rules or retrieving outdated patterns. Optional: use RAG to find existing docs on similar topics to avoid duplication, but keep the style guide itself in direct context.

</Collapser>

<Collapser title="Reviewing">
...
<Collapser title="Discovering (Finding & Answering)">
...
**Choosing between RAG and direct context loading:**

- **RAG**: Best for large doc sets that exceed context windows, open-ended questions across entire corpus, unpredictable user queries. Requires vector DB infrastructure and indexing pipeline. Can have retrieval quality issues.
- **Direct context loading**: Works when docs fit in modern context windows (100k+ tokens). More accurate since full page structure is preserved. Simpler architecture - just serve markdown via API or MCP. Better for page-specific questions.
- **Hybrid approach**: Load current page + related pages directly, use RAG for broader corpus search. Let users explicitly add pages to context.
...
**Emerging patterns for AI integration on docs sites:**

- **Copy page as markdown**: Allow users to copy the current page in markdown format for use in LLM/AI applications
- **Markdown URL endpoints**: Serve markdown versions at the same URL path with `.md` extension (e.g., `/docs/guide.md`) for easy LLM retrieval
- **Contextual AI chat**: "Ask AI about this page" buttons that auto-populate the chat with page context
- **Code block AI features**: "Explain this code" buttons within code examples for inline assistance
...
<Callout variant="tip">
  Best used as a supplement to human-authored docs, not a complete replacement.
</Callout>
