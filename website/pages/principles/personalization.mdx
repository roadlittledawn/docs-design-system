---
title: Personalization
---

Personalization features help users navigate documentation more efficiently by surfacing relevant content based on their behavior and preferences.

<CardGrid columns={2}>
  <Card title="Recently Viewed">
    Track and display pages users have recently visited, enabling quick return to previously accessed content.
  </Card>

<Card title="Recently Searched For">
  Show users their recent search queries, allowing them to quickly re-run
  searches or refine previous queries.
</Card>

<Card title="Code Examples">
  Personalize code examples based on user preferences such as programming
  language, framework, or SDK version.
</Card>

<Card title="Recommended Content">
  Suggest relevant documentation based on user behavior, current page context,
  and common navigation patterns.
</Card>

  <Card title="Favorite Pages">
    Allow users to bookmark and quickly access frequently referenced documentation pages.
  </Card>
</CardGrid>
